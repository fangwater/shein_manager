...
func (client *Client) QueryInventory(ctx context.Context, items []InventoryItem) (json.RawMessage, error) {
	...
	if err := client.do(ctx, http.MethodPost, "/temu/warehouse-availability/query", body, "", &result); err != nil {
		return nil, err
	}
	return result, nil
}

func (client *Client) do(ctx context.Context, method, path string, body []byte, account string, target any) error {
